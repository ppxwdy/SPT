...
class VNode():
    ...
    def __init__(self, node, dx) -> None:
        ...
        self.message_send = {}                          # receiver: message send. FacNode: ndarray
        self.marginal = np.ones(len(dx)) / len(dx)  # normalized marginal distribution
        # self.neighbors = []
    ...
    def send(self, rec, test=False):
        ...
        message2 = np.prod([np.ones(len(self.dx))]+[v for k,v in self.message_rec.items() if k != rec], axis=0)
        # message2 = np.prod(message2, axis=0)
        message2 /= sum(message2)
        rec.receive(self, message2/sum(message2))
        self.message_send[rec] = message2/sum(message2)
        # print(mess2/sum(mess2), message1) 
    ...
    def update(self):
        ...
        """
        self.marginal = np.prod([np.ones(len(self.dx))]+[v for v in self.message_rec.values()], axis=0)
...
class FNode():
    ...
    def initialize(self, node1, node2, func=quadratic, weight=1):
        ...
        #         state1 = states1[i]
        #         state2 = states2[j]
        #         p = func(state1, state2) # np.exp(-(state1-state2)**2)
        #         potentials[i][j] = p

        
        Dx1, Dx2 = node1.dx, node2.dx
        l = len(Dx1)
        ...
        for i in range(l):
            ...
            for j in range(l):
                d2 = Dx2[j]
                p = np.exp(- (min((i - j)**2, 100000))) #np.exp(- d1 - d2 - (min(10 * (i - j)**2, 8)))
                potentials[i][j] = p
        
        # self.joint_dis = np.array(potentials)   
        self.joint_dis = np.array(potentials)
        self.nodes = [node1, node2]
        # print(self.joint_dis)
    ...
    def send(self, rec, test=False):
        ...
        out = np.zeros(len(rec.dx))
        if len(self.nodes) == 1:
            # for row in range(duplicate.shape[0]):
            #     out[row] = duplicate[row]
        ...
        # send to node 1, up 
            ...
            # print(self.name, "message is ", message)
        ...
        out = out**self.weight
        final_message = out/sum(out) # self.weight * out/sum(out)
...
class factor_tree():
    ...
    def add_node(self, node:int, dx, max_var=0):
        """
    ...
    def add_nodes(self, nodes, Dxs, max_vars=[]):
        """_summary_

        Args:
            nodes (_type_): list of int
            state_nums (_type_): list of ndarray
        """
        for node, Dx, max_var in zip(nodes, Dxs, max_vars):
            self.add_node(node, Dx, max_var)
